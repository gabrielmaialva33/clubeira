defmodule ClubeiraWeb.Member.OrderController do
  ...
  def index(conn, %{"polo_slug" => polo_slug} = params) do
    with {:ok, route} <- Polos.resolve_route(polo_slug),
         {:ok, result} <-
           Billing.list_orders(order_scope(conn, route.polo_id), pagination_params(params)) do
      render(conn, :index, orders: result.orders, page: result.page)
    else
      ...
      {:error, reason} ->
        Logger.error("could not list member orders: #{inspect(reason)}")
        render_error(conn, :service_unavailable)
    end
  end

  defp order_scope(conn, polo_id) do
