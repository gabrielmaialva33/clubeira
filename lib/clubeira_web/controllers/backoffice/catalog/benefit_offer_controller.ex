defmodule ClubeiraWeb.Backoffice.BenefitOfferController do
  ...
  @offer_fields ~w(code name benefit_kind)
  @version_fields ~w(
  ...
  def create(conn, %{"polo_slug" => polo_slug, "place_id" => place_id} = params) do
    with {:ok, idempotency_key} <- fetch_idempotency_key(conn),
      ...
      {:error, :offer_code_taken} ->
        render_error(conn, :conflict, "benefit_offer_code_conflict")
  ...
  defp publishing_attributes(params, idempotency_key) do
    offer = map_param(params, "offer")
    version = map_param(params, "version")
    effective_during = map_param(version, "effective_during")

    %{
      "offer" => Map.take(offer, @offer_fields),
      "version" =>
        version
