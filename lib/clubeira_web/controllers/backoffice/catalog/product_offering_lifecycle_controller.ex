defmodule ClubeiraWeb.Backoffice.ProductOfferingLifecycleController do
  ...
  def create(conn, %{"polo_slug" => polo_slug, "product_offering_id" => offering_id} = params) do
    with {:ok, idempotency_key} <- fetch_idempotency_key(conn),
      ...
      {:error, reason} when reason in [:polo_not_found, :product_offering_not_found] ->
      ...
      {:error, :invalid_product_offering_transition} ->
        render_error(conn, :conflict, "invalid_product_offering_transition")
