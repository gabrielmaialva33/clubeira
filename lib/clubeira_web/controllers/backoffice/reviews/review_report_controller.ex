defmodule ClubeiraWeb.Backoffice.ReviewReportController do
  ...
  @resolution_fields ~w(action reason)

  def index(conn, %{"polo_slug" => polo_slug} = params) do
    with {:ok, route} <- Polos.resolve_route(polo_slug),
         {:ok, page} <- Reviews.list_reports(scope(conn, route.polo_id), params) do
      render(conn, :index, reports: page.reports, page: page.page)
    else
  ...
  def create_action(
        conn,
        %{"polo_slug" => polo_slug, "review_report_id" => review_report_id} = params
      ) do
    with {:ok, idempotency_key} <- fetch_idempotency_key(conn),
         ...
         {:ok, result} <-
           Reviews.resolve_report(
             ...
             resolution_attributes(params, review_report_id, idempotency_key)
           ) do
      conn
      |> put_status(:created)
      |> render(:create_action, result: result)
    else
      {:error, reason} when reason in [:polo_not_found, :review_report_not_found] ->
        render_error(conn, :not_found)
  ...
  defp resolution_attributes(params, review_report_id, idempotency_key) do
    params
    |> Map.take(@resolution_fields)
    |> Map.put("review_report_id", review_report_id)
