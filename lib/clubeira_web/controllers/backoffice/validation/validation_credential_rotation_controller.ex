defmodule ClubeiraWeb.Backoffice.ValidationCredentialRotationController do
  ...
  def create(conn, %{"polo_slug" => polo_slug, "credential_id" => credential_id} = params) do
    with {:ok, idempotency_key} <- fetch_idempotency_key(conn),
         ...
         {:ok, result} <-
           Redemptions.rotate_validation_credential(
             ...
             credential_id,
             rotation_attributes(params, idempotency_key)
           ) do
      conn
      |> put_status(:created)
  ...
  defp rotation_attributes(params, idempotency_key) do
    params
    |> map_param("credential")
    |> Map.take(~w(secret_sha256 expires_at))
