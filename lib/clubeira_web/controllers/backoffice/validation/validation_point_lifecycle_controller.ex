defmodule ClubeiraWeb.Backoffice.ValidationPointLifecycleController do
  ...
  def create(conn, %{"polo_slug" => polo_slug, "validation_point_id" => point_id} = params) do
    with {:ok, idempotency_key} <- fetch_idempotency_key(conn),
      ...
      {:error, reason} when reason in [:polo_not_found, :validation_point_not_found] ->
      ...
      {:error, :invalid_validation_point_transition} ->
        render_error(conn, :conflict, "invalid_validation_point_transition")

      {:error, :validation_point_unavailable} ->
        render_error(conn, :conflict, "validation_point_unavailable")
