defmodule ClubeiraWeb.Backoffice.PlaceController do
  ...
  @query_fields ~w(after limit place_id profile_status status)

  def index(conn, %{"polo_slug" => polo_slug} = params) do
    with {:ok, route} <- Polos.resolve_route(polo_slug),
         {:ok, result} <-
           Directory.list_backoffice_places(
             scope(conn, route.polo_id),
             Map.take(params, @query_fields)
           ) do
      render(conn, :index, places: result.places, page: result.page)
    else
      ...
      {:error, reason}
      when reason in [:invalid_place_id, :invalid_place_profile_status, :invalid_place_status] ->
        render_error(conn, :unprocessable_entity)

      {:error, reason} ->
        Logger.error("could not list backoffice places: #{inspect(reason)}")
