defmodule ClubeiraWeb.CoreComponents do
  @moduledoc """
  ...
  Components are styled directly with Tailwind CSS utilities so their behavior
  and visual contract remain owned by Clubeira.
    ...
    <ul class="divide-y divide-clubeira-line rounded-xl border border-clubeira-line bg-white dark:divide-slate-800 dark:border-slate-800">
      <li :for={item <- @item} class="px-4 py-3">
        <div>
          ...
          <div class="mt-1 text-sm text-slate-600 dark:text-slate-400">{render_slot(item)}</div>
        </div>
      </li>
    </ul>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ClubeiraWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ClubeiraWeb.Gettext, "errors", msg, opts)
    end
  end
